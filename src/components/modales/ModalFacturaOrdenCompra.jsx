...
export const ModalFacturaOrdenCompra = ({ show, handleClose, idOrdenCompra, onSave }) => {
    ...
    useEffect(() => {
        if (idOrdenCompra) {
            ...
            axiosInstance.get(`/ordenes-compra/${idOrdenCompra}`)
                .then(response => {
            ...
            axiosInstance.get(`/ordenes-compra/${idOrdenCompra}/detalles`)
                .then(response => {
                    setProductos(response.data);
                })
                .catch(error => {
                    toast.error("Error al cargar los productos de la orden de compra");
                    console.error("Error al cargar los productos de la orden de compra:", error);
                });
        }
    ...
    const handleGuardar = async () => {
        ...
        if (!facturaRegex.test(numeroFactura)) {
            toast.error("Formato de número de factura incorrecto. Ej: 001-001-0000001");
            return;
        }
        ...
        if (!fechaEmision || !fechaVencimiento || !numeroFactura || !timbrado || !condicion || !deposito) {
        ...
        try {
            ...
            const promesas = productos.map(producto => {
                ...
                console.log("Enviando producto:", JSON.stringify(detalleFactura, null, 2)); // Mensaje de depuración

                return axiosInstance.post(`/facturas-detalles/${facturaId}`, detalleFactura);
            });
            ...
            console.error("Error al guardar la factura o los productos:", error.response?.data || error.message);
                ...
                <Modal.Body>
                                ...
                                <tr key={index}>
                                    ...
                                    <td>
                                        <input
                                            ...
                                            onChange={(e) => {
                                                const newProductos = [...productos];
                                                newProductos[index].producto.descripcion = e.target.value;
                                                setProductos(newProductos);
                                            }}
                                        />
                                    </td>
                    ...
                    <div className="form-group">
                        ...
                        <input
                            type="text"
                            className="form-control"
                            value={(productos.reduce((acc, prod) => acc + (prod.precioUnitario * prod.cantidad), 0) / 11).toFixed(0)}
                            readOnly
                        />
                    </div>
                    <div className="form-group">
                        ...
                        <input
                            type="text"
                            className="form-control"
                            value={productos.reduce((acc, prod) => acc + (prod.precioUnitario * prod.cantidad), 0)}
                            readOnly
                        />
                    </div>
                </Modal.Body>
